                ...
                >
                  <template v-for="(tag, tIndex) in row" :key="tag">
                    <span
                      ...
                      :class="
                        item.hoverBorder +
                        ' px-3 max-[361px]:px-2 py-1.5 border border-[#2e2e2e] rounded-full text-zinc-300 bg-[#171717] text-[clamp(13px,2.9vw,16px)]'
                      "
                    >
                      {{ tag }}
                    </span>
...
const updateDots = () => {
  Object.keys(dotRefs).forEach((key) => {
    ...
    dot.style.display =
      tag.offsetTop === nextTag.offsetTop ? "inline-block" : "none";
  });
};
